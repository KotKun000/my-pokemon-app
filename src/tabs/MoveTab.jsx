...
function MoveTab() {
  ...
  return (
    <>
                  ...
                  >
                    <img
                      ...
                      onError={(event) => {
                        ...
                        event.currentTarget.src = getFallbackTypeIconUrl(suggestion.type);
                      }}
                    />
                    {suggestion.name}
                  </button>
      ...
      {!searched.trim() && (
        <p className="status-message">พิมพ์คำค้นแล้วกดปุ่มค้นหาเพื่อดูข้อมูล Move</p>
      )}

      {searched.trim() && filteredMoves.length === 0 && (
        <p className="status-message">ไม่พบท่าที่ตรงกับคำค้น `{searched}`</p>
      )}
